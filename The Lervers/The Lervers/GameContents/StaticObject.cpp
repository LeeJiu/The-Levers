...
void StaticObject::draw( int x, int y, const Image* image ) const {
   ...
   bool floor = false;
   if ( mFlags & Normal::FLAG_WALL_W ){ //벽가로
       srcX = 64; 
       srcY = 0;
   }else if ( mFlags & Normal::FLAG_WALL_H ){   //벽 세로
      srcX = 96; 
      srcY = 0;
   }else if ( mFlags & Normal::FLAG_HOLE ){   //구멍
      srcX = 0; 
      srcY = 32;
   }else if ( ( mFlags & Lights::FLAG_LIGHT1 ) || ( mFlags & Lights::FLAG_LIGHT2 ) || ( mFlags & Lights::FLAG_LIGHT3 )
      || ( mFlags & Lights::FLAG_LIGHT4 ) || ( mFlags & Lights::FLAG_LIGHT5 ) ){   //빛 off 상태
      srcX = 96; 
      srcY = 32;
   }else if ( !( mFlags & Lights::FLAG_LIGHT1 ) || !( mFlags & Lights::FLAG_LIGHT2 ) || !( mFlags & Lights::FLAG_LIGHT3 )
      || !( mFlags & Lights::FLAG_LIGHT4 ) || !( mFlags & Lights::FLAG_LIGHT5 ) ){   //빛 on 상태
      srcX = 64; 
      srcY = 32;
   ...
   if ( floor ){
      ...
      || ( mFlags & Levers::FLAG_LEVER4 ) || ( mFlags & Levers::FLAG_LEVER5 ) ){   //레버 off 상태
      srcX = 32; 
      srcY = 64;
   }else if ( ( mFlags & Levers::FLAG_LEVER1 ) || ( mFlags & Levers::FLAG_LEVER2 ) || ( mFlags & Levers::FLAG_LEVER3 )
      || ( mFlags & Levers::FLAG_LEVER4 ) || ( mFlags & Levers::FLAG_LEVER5 ) ){   //레버 on 상태
      srcX = 0; 
      srcY = 64;
   }
      if ( srcX != -1 ){
