...
public:
	...
	static void create();
	static void destroy();
	bool hasLoaded(); //ロ?ド終わった？

	enum Bgm{
		BGM_TITLE, //ﾅｸﾀﾌﾆｲ ｺ�ｱﾝ
		BGM_GAME, //ｰﾔﾀﾓｺ�ｱﾝ
	...
	enum Se{
		SE_DEATH, //ｻ邵ﾁｽﾃ ｼﾒｸｮ
		SE_CURSOR_MOVE, //ﾄｿｼｭ ｼﾒｸｮ
		SE_SELECTION, //ｼｱﾅﾃｼﾒｸｮ
		SE_SET_LEVER, //ｷｹｹﾒｸｮ

		SE_MAX,
	};
	//BGMを鳴らす。BGMは同時に一個しか鳴らない。また、勝手にル?プする。
	void playBgm( Bgm );
	void stopBgm();
...
private:
	...
	GameLib::Sound::Player mSePlayers[ SE_PLAYER_MAX ]; //これは配列で毎フレ???ェックして終わってたら削除。
